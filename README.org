...
- Tabby Agent (LSP server): Requires [[https://nodejs.org/en/download/][Node.js]] version v18.0+ and [[https://www.npmjs.com/package/tabby-agent][tabby-agent]] installed (=npm install --global tabby-agent=)
  - Remember to configure Tabby Agent according to [[https://tabby.tabbyml.com/docs/extensions/configurations/][documentation]], especially set =endpoint= and =token= values in =~/.tabby-client/agent/config.toml= file.
...
*** Custom keybings
You can also override default keybindings maps:
- =tabbymacs-mode-map=
  - =tabbymacs-accept-ghost-text=
  - =tabbymacs-cancel-ghost-text=
- =tabbymacs-overlay-map=
  - =tabbymacs-invoked-inline-completion=
    
**** Example
#+begin_src emacs-lisp
;;Example keybindings override configuration
(use-package tabbymacs
  :bind
  (:map tabbymacs-mode-map
        ("C-c \\" . tabbymacs-invoked-inline-completion)
        ("C-c /" . nil)
   :map tabbymacs-overlay-map
        ("<tab>" . nil)))
#+end_src
